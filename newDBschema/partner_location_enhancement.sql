-- =====================================================
-- PARTNER AND LOCATION ENHANCEMENT SCRIPT
-- =====================================================
-- This script improves the partners and addresses tables structure
-- and adds sample data for map markers and detailed partner profiles

-- Enable UUID extension if not enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Check if partner_locations exists as a table and drop it if needed
DO $$
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'Dropped existing partner_locations table to replace with view';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'partners') THEN
    ...
    EXECUTE 'CREATE INDEX idx_partners_business_name ON partners (business_name)';
    EXECUTE 'CREATE INDEX idx_partners_is_facility ON partners (is_facility)';
    
    RAISE NOTICE 'Created partners table';
  ELSE
    -- If table exists, add any missing columns
    BEGIN
      EXECUTE 'ALTER TABLE partners 
      ...
      RAISE NOTICE 'Updated partners table with missing columns';
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'addresses') THEN
    ...
    EXECUTE 'CREATE INDEX idx_addresses_latitude_longitude ON addresses (latitude, longitude)';
    EXECUTE 'CREATE INDEX idx_addresses_partner_id ON addresses (partner_id)';
    EXECUTE 'CREATE INDEX idx_addresses_is_facility ON addresses (is_facility)';
    
    RAISE NOTICE 'Created addresses table';
  ELSE
    -- If table exists, add any missing columns
    BEGIN
      ...
      RAISE NOTICE 'Updated addresses table with missing columns';
    EXCEPTION WHEN OTHERS THEN
...
-- Insert sample address data for partners
...
-- =====================================================
-- DROP AND RECREATE VIEW FOR COMBINED PARTNER LOCATIONS
-- =====================================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    EXECUTE 'DROP VIEW public.partner_locations CASCADE';
    RAISE NOTICE 'Dropped existing partner_locations view to recreate it';
  END IF;
END$$;

-- Create a view for easy access to partner locations with all details
...
ALTER VIEW partner_locations OWNER TO postgres;
...
-- ENABLE RLS on partners and addresses? (Probably not for public data)
ALTER TABLE partners ENABLE ROW LEVEL SECURITY;
ALTER TABLE addresses ENABLE ROW LEVEL SECURITY;

-- Create policies to allow public read access to partners and addresses
